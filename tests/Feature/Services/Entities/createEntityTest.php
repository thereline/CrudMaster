...
it('creates a entity with no relationships successfully', function () {
    ...
    // When
    $this->repository->setCreateEntityClass(new School);
    $result = $this->repository->createEntity($data);
...
it('creates a entity with array relationships with pivot successfully', function () {
    // Given a School and related Students
    $data = ['name' => 'Test School', 'email' => 'test@example.com'];
    $relationships = [
        'students' => [
    ...
    // When
    $this->repository->setCreateEntityClass(new School);
    $result = $this->repository->createEntity($data, $relationships);
...
it('creates a entity with relationships without pivot values successfully', function () {
    // Given a School and related Students
    $data = ['name' => 'Test School', 'email' => 'test@example.com'];
    $relationships = [
        'teachers' => [
            ['name' => 'Teacher One', 'active' => true],
        ],
    ];
    ...
    expect($result)
        ...
        ->and($result['name'])->toEqual('Test School')
        // Ensure departments are created
        ->and($result['teachers'])->toHaveLength(1)
        ->and($result['teachers'][0]['name'])->toEqual('Teacher One');
...
it('handles unique constraint violations for relationships', function () {
    ...
    // When
    $this->repository->setCreateEntityClass($mockModel);
    $result = $this->repository->createEntity($data, $relationships);
